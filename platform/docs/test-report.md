...
`platform` 与 V3 桌面程序已经完成连接：用户只用邮箱和密码注册或登录，登录会话通过 Windows 安全存储加密持久化；点击 Online Host 后会自动登记设备、完成一次性配对、创建并启用 Host。

本地阶段的 `/h/{slug}/v1/*` 数据面已经可用，平台会把三条 OpenAI 兼容路由转发到本机 Gateway，保留普通响应、SSE、鉴权错误与 Request ID。当前地址仍是 localhost，不能从其他设备访问；真正公网化仍需要部署平台和出站 Relay。
...
| 层级 | 结果 | 覆盖内容 |
| --- | --- | --- |
| 桌面 Gateway Node 测试 | 97/97 通过 | 平台会话、HTTPS 边界、自动配对、OpenAI 兼容层、图片、流式响应、API Key、用量与既有桌面功能 |
| 桌面 JavaScript 语法检查 | 55 个文件通过 | 主进程、preload、页面和脚本 |
| Spring Boot 单元/集成测试（H2） | 16/16 通过 | API、鉴权、CSRF、限流、资源归属、配置、错误处理、加密令牌、OpenAPI 与本地 Relay |
...
| V3 本地 Host 端到端 | 通过 | 桌面登录、自动设备配对、Host 启用、`/v1/models` 401 鉴权与 Request ID |
| 桌面视觉回归 | 通过 | 中英文、账号弹窗、Share online、Online Host、设置页无 Tailscale 控件、暗色/亮色与移动布局 |
...
- Nginx 暴露前端、`/api/*` 与严格限制的 `/h/{slug}/v1/*`；数据库与 localhost 预览入口仅绑定 loopback。
...
- localhost 代理开启时 Host 会立即反映 online/offline；生产模式仍由 Relay presence 决定状态。
...
7. Docker Desktop 发往 Gateway 的 Host 原为 `host.docker.internal`，触发本地 Host 防护；本地代理现在安全地改写为 `127.0.0.1:4310`。
8. Java HTTP Client 默认尝试 `h2c` 升级，曾被 Gateway WebSocket 处理器识别为升级请求并返回 404；本地代理现在固定使用 HTTP/1.1，普通与 SSE 请求均走正确路由。
