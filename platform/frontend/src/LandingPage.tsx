      ...
      <section className="landing-content-section" id="overview" data-landing-section="overview">
        <div className="landing-section-shell">
          ...
          <div className="feature-grid">
            ...
            <article className="feature-card wide landing-reveal"><small>04 / {t('平台', 'Platform')}</small><b>{t('V3 基础', 'V3 foundation')}</b><h3>{t('从本机 Host，走向稳定的跨设备入口。', 'From a local Host to a stable cross-device entry point.')}</h3><p>{t('V3 已加入平台账号、持久登录、设备登记、自动配对与稳定 Host 分配。当前公开 Relay 尚未部署，真正远程访问仍需要 Relay 上线。', 'V3 adds accounts, persistent sessions, device enrollment, automatic pairing and stable Host allocation. The public Relay is not deployed yet and is still required for real remote access.')}</p><MiniVisual type="platform" t={t}/></article>
          </div>
        ...
        <div className="landing-section-shell">
          ...
          <div className="connect-layout">
            ...
            <div className="code-window landing-reveal">
              ...
              <div className="host-switch"><div><button className={hostMode === 'local' ? 'active' : ''} onClick={() => setHostMode('local')}>{t('本地 Host', 'Local Host')}</button><button className={hostMode === 'online' ? 'active' : ''} onClick={() => setHostMode('online')}>Online Host</button></div><span className={hostMode === 'online' ? 'warning' : ''}><i/>{hostMode === 'online' ? t('需要部署公网 Relay', 'Requires a deployed public Relay') : t('本机可用', 'Available locally')}</span></div>
